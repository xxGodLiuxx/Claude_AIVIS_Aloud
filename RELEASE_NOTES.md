...
### 概要
JaH対応版。音声読み上げの自然さを大幅に向上させ、開発者固有の発音やリスト処理を最適化。
...
#### 2. **音量差別化の強化**
- 通常応答: `VOLUME_NORMAL = 1.0`
- Thinking部分: `VOLUME_THINKING = 0.5`（v3.1.4の0.6からさらに控えめに）

#### 3. **テキスト処理の改良**
- **番号付きリスト**: `1. 項目` → `1、項目`（自然な読み上げ形式）
- **括弧処理**: 全ての括弧を句点に変換し、音声の区切りを明確化
  - `（）` `()` `「」` `『』` → すべて `。` に変換

#### 4. **JSONL監視の継続強化**
- 動的ファイル切り替え機能（v3.1.4から継承）
- 10秒間隔での新セッション自動検出
- 6段階の完全な切り替えプロセス

### 技術的詳細
- ファイル監視戦略: 最後の10KBから読み取り開始
- 重複防止: deque(maxlen=100)によるメッセージIDキャッシュ
- エラー処理: ファイル削除・未初期化ハンドル対応

### 改善
- CLAUDE.mdバージョン管理規則の追加
- セマンティックバージョニングの採用

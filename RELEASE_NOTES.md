...
## v3.3.0 (2025-09-03) 🎉 最新版

### 概要
統合版。v3.2.1のエンタープライズ機能とv3.1.5の監視改良を統合した最強バージョン。

### 主な特徴

#### 1. **エンタープライズ機能（v3.2.1から継承）**
- Claude Code CLI最適化
- バックグラウンド実行サポート
...
#### 2. **JSONL監視強化（v3.1.5から統合）**
- 6段階の明確なセッション切り替えプロセス
- メッセージIDキャッシュの適切なクリア
- 「新しいセッションが始まりました」音声通知
- 10秒間隔での新セッション自動検出

#### 3. **テキスト処理改良（両バージョンから統合）**
- 番号付きリスト: `1. 項目` → `1、項目`
- 括弧の音声区切り: `（）` `()` `「」` `『』` → `。`
- datetime形式の自然な読み上げ
...
### アップグレード推奨度: ⭐⭐⭐⭐⭐
全ユーザーに推奨。v3.1.xとv3.2.xの良い点を統合した決定版。

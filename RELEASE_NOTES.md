...
#### 1. **Claude Code Desktop完全対応**
- Desktop版JSONL形式のネイティブサポート
- サブエージェントJSONLの自動除外（二重読み上げ防止）
- 重複プロセス検出のスクリプト名対応

#### 2. **ツール実行の日本語読み上げ（新機能）**
- Bash, Read, Write, Edit, Glob, Grep, Agent等すべてのツールを日本語で通知
- ファイル名・パスは読み上げず、ファイル種類と動作だけを簡潔に伝達
- 例：「パイソンファイルを編集します」「コマンドを実行します」

#### 3. **内部思考の自然な日本語変換（大幅改善）**
- 英語の思考内容を自動で日本語に変換
- パス・コード片・長い識別子を自動除去
- 日本語が多い思考はライトクリーンアップのみ

#### 4. **ユーザー入力確認・許可通知（新機能）**
- ユーザーがコマンドを入力したら「受け取りました」と読み上げ
- ツール使用が拒否されたら「ユーザーが操作を拒否しました」と通知

#### 5. **テキスト処理の強化**
- 技術用語辞書を大幅拡充（100語以上）
- 日本語文字境界でも正しく用語置換（\bの制限を克服）
- インラインコード・バッククォートの自動除去
- URL・ファイルパスの自動除去
- 残留英単語の自動クリーンアップ

#### 6. **音量設定の調整**
- 通常応答: 0.3（控えめ）
- 思考・ツール通知: 0.1（バックグラウンド的）

### アップグレード推奨度: ⭐⭐⭐⭐⭐
Desktop版ユーザーには必須。耳だけでClaude Codeの動作を把握可能に。
